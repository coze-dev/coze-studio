...
struct CreateSliceRequest {
    2: required i64 document_id(api.js_conv="str")
    5: optional string raw_text
    6: optional i64 sequence(api.js_conv="str")
...
struct CreateSliceResponse {
    1: i64  slice_id(api.js_conv="str")
...
struct UpdateSliceRequest {
    2: required i64 slice_id (api.js_conv="str")
...
struct ListSliceRequest {
    2:  optional i64    document_id(api.js_conv="str")
    3:  optional i64    sequence(api.js_conv="str")     // 序号
    4:  optional string keyword                         // 查询关键字
    5:  optional i64    dataset_id (api.js_conv="str")  // 如果只传 dataset_id，则返回该知识库下的分片

    20:          i64    page_no(api.js_conv="str")    // 从1开始
    21:          i64    page_size(api.js_conv="str")
...
struct SliceInfo {
    ...
    4: i64         hit_count(api.js_conv="str")   // 命中次数
    5: i64         char_count(api.js_conv="str")  // 字符数
    6: i64         token_count(api.js_conv="str") // token数
    7: i64         sequence(api.js_conv="str")    // 序号
    8: i64         document_id(api.js_conv="str")
