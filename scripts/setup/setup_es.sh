...
ES_TEMPLATES=$(find "$SCRIPT_DIR/es_index_schema" -type f -name "*.index-template.json" | sort)
if [ -z "$ES_TEMPLATES" ]; then
  echo -e "${YELLOW}ℹ️ No Elasticsearch index templates found in $SCRIPT_DIR/es_index_schema${NC}"
else
  # 新增索引创建逻辑
