...
func (p *pluginSyncerImpl) SyncToolToPluginOpenapiDoc(ctx context.Context, updated, tool *entity.ToolInfo) (newPlugin *entity.PluginInfo, err error) {
	...
	urlPath := tool.GetSubURLPath()
	if updated.SubURLPath != nil {
		urlPath = updated.GetSubURLPath()
	}

	pathItem, _ := p.OpenapiDoc.Paths[urlPath]

	method := tool.GetReqMethodName()
	if updated.ReqMethod != nil && updated.GetReqMethodName() != tool.GetReqMethodName() {
		method = updated.GetReqMethodName()
		pathItem.SetOperation(updated.GetReqMethodName(), pathItem.GetOperation(tool.GetReqMethodName()))
	}
...
func (p *pluginSyncerImpl) activateTool(tool *entity.ToolInfo) {
	...
	p.OpenapiDoc.Paths[tool.GetSubURLPath()] = pathItem
}
...
func (p *pluginSyncerImpl) deactivateTool(updated, tool *entity.ToolInfo) {
	...
	delete(p.OpenapiDoc.Paths, tool.GetSubURLPath())

	return
}

func (p *pluginSyncerImpl) syncPath(updated, tool *entity.ToolInfo) {
	if updated.SubURLPath == nil || updated.GetSubURLPath() == tool.GetSubURLPath() {
		return
	}

	paths := p.OpenapiDoc.Paths

	pathItem := paths[updated.GetSubURLPath()]
	if pathItem != nil {
		return
	}

	pathItem, ok := paths[tool.GetSubURLPath()]
	if !ok {
		paths[updated.GetSubURLPath()] = &openapi3.PathItem{}
		return
	}

	delete(paths, tool.GetSubURLPath())
	paths[updated.GetSubURLPath()] = pathItem
}

func (p *pluginSyncerImpl) syncRequestParams(urlPath, method string, reqParams []*plugin_common.APIParameter) {
	op := p.OpenapiDoc.Paths[urlPath].Operations()[method]

	params := make([]*openapi3.ParameterRef, 0)
	subParams := make([]*plugin_common.APIParameter, 0)

	for _, param := range reqParams {
		if param.Location == plugin_common.ParameterLocation_Body {
	...
	mediaType.Schema = convertAPIParamToOpenapi3Schema(&plugin_common.APIParameter{
		Type:          plugin_common.ParameterType_Object,
...
func (p *pluginSyncerImpl) syncResponseParams(urlPath, method string, respParams []*plugin_common.APIParameter) {
	op := p.OpenapiDoc.Paths[urlPath].Operations()[method]
...
func convertAPIParamToOpenapi3Schema(param *plugin_common.APIParameter) *openapi3.SchemaRef {
	if param == nil {
		return nil
	}
...
func unmarshalGlobalDefault(s string, typ plugin_common.ParameterType) any {
	if s == "" {
		return nil
	}
	...
	case plugin_common.ParameterType_Integer, plugin_common.ParameterType_Number:
...
func (p *pluginSyncerImpl) ApplyPluginOpenapi3DocToTools(ctx context.Context,
	...
	for _, oldTool := range oldTools {
		...
		k := fmt.Sprintf("%s:%s", oldTool.GetSubURLPath(), method)
	...
	for k, pathItem := range pathItems {
		...
		newTool := &entity.ToolInfo{
			...
			DebugStatus:     ptr.Of(plugin_common.APIDebugStatus_DebugWaiting),
			ReqMethod:       ptr.Of(convertor.ToThriftAPIMethod(method)),
			SubURLPath:      ptr.Of(subURLPath),
		}
...
func getAPIReqParamsFromOperation(ctx context.Context, op *openapi3.Operation) ([]*plugin_common.APIParameter, error) {
	reqAPIParams := make([]*plugin_common.APIParameter, 0, len(op.Parameters))
	...
	if op.RequestBody != nil &&
		...
		for _, ct := range contentTypeArray {
			...
			apiParam, err := convertOpenapi3SchemaToAPIParam(ctx, mType.Schema.Value, plugin_common.ParameterLocation_Body)
			if err != nil {
				return nil, err
			}
...
func getAPIRespParamsFromOperation(ctx context.Context, op *openapi3.Operation) ([]*plugin_common.APIParameter, error) {
	...
	for code, v := range op.Responses {
		...
		for _, ct := range contentTypeArray {
			...
			respAPIParam, err := convertOpenapi3SchemaToAPIParam(ctx, mType.Schema.Value, plugin_common.ParameterLocation_Body)
			if err != nil {
				return nil, err
			}

			return []*plugin_common.APIParameter{respAPIParam}, nil
		}
	}
...
func convertOpenapi3ParamToAPIParam(ctx context.Context, param *openapi3.Parameter) (*plugin_common.APIParameter, error) {
	if param == nil {
		return nil, fmt.Errorf("ParameterRef is nil")
	}
	...
	res := &plugin_common.APIParameter{
		Name:       param.Name,
	...
	if sub.Type == plugin_common.ParameterType_Object || sub.Type == plugin_common.ParameterType_Array {
...
func convertOpenapi3SchemaToAPIParam(ctx context.Context, sc *openapi3.Schema, location plugin_common.ParameterLocation) (*plugin_common.APIParameter, error) {
	if sc == nil {
	...
	if sc.Type == openapi3.TypeObject && len(sc.Properties) > 0 {
		...
		if item != nil && item.Type == plugin_common.ParameterType_Object {
			if res.SubType == nil {
				res.SubType = ptr.Of(plugin_common.ParameterType_Object)
			}

			res.SubParameters = append(res.SubParameters, item.SubParameters...)
		}

		if item != nil && item.Type != plugin_common.ParameterType_Array && item.Type != plugin_common.ParameterType_Object {
...
func isParamsNotEqual(oldParams, newParams []*plugin_common.APIParameter) bool {
	...
	newParamsMap := make(map[string]*plugin_common.APIParameter)
