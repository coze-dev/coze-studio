...
func (l *standardOAuth) getAccessTokenByAuthorizationCode(ctx context.Context, ci *entity.AuthorizationCodeInfo) (accessToken string, err error) {
	...
	info, exist, err := l.oauthRepo.GetAuthorizationCodeInfo(ctx, ci.Meta)
	if err != nil {
		return "", errorx.Wrapf(err, "[standardOAuth] GetAuthorizationCodeInfo failed, userID=%s, pluginID=%d, isDraft=%t",
