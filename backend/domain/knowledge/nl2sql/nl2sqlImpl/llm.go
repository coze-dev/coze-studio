...
func (r *NL2Sql) NL2Sql(ctx context.Context, query string, chatHistory []*schema.Message, tableMeta []*nl2sql.DBTableMetadata) (sqlString string, err error) {
	if len(tableMeta) == 0 {
		return "", errors.New("table meta is empty")
	}
	tableDesc := ""
	for _, table := range tableMeta {
		tableDesc += fmt.Sprintf(defaultTableFmt, table.TableName, table.Comment)
