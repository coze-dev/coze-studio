...
func (i *invokableWorkflow) InvokableRun(ctx context.Context, argumentsInJSON string, opts ...tool.Option) (string, error) {
	...
	if rInfo != nil {
		cancelCtx, executeID, callOpts, err = Prepare(ctx, "", i.wfEntity.GetBasic(i.sc.NodeCount()),
			rInfo, i.repo, i.sc,
		...
		cancelCtx, executeID, callOpts, err = Prepare(ctx, argumentsInJSON, i.wfEntity.GetBasic(i.sc.NodeCount()),
			nil, i.repo, i.sc,
			execute.GetIntermediateStreamWriter(opts...), cfg)
		if err != nil {
			return "", err
...
func (s *streamableWorkflow) StreamableRun(ctx context.Context, argumentsInJSON string, opts ...tool.Option) (*schema.StreamReader[string], error) {
	...
	if rInfo != nil {
		cancelCtx, executeID, callOpts, err = Prepare(ctx, "", s.wfEntity.GetBasic(s.sc.NodeCount()),
			rInfo, s.repo, s.sc,
			execute.GetIntermediateStreamWriter(opts...), cfg)
		if err != nil {
			return nil, err
		...
		cancelCtx, executeID, callOpts, err = Prepare(ctx, argumentsInJSON, s.wfEntity.GetBasic(s.sc.NodeCount()),
			nil, s.repo, s.sc,
			execute.GetIntermediateStreamWriter(opts...), cfg)
		if err != nil {
			return nil, err
