...
type mockDsSelect struct {
	t        *testing.T
	objects  []Object
	validate func(request *QueryRequest)
}

func (m *mockDsSelect) Query(ctx context.Context, request *QueryRequest) (*Response, error) {
	n := 1
...
func TestDataset_Query(t *testing.T) {
	t.Run("string case", func(t *testing.T) {
		...
		t.Run("single", func(t *testing.T) {
			...
			cfg := &QueryConfig{
				...
				OrderClauses: []*OrderClause{{FieldID: "v1", IsAsc: false}},
			...
			mockQuery := &mockDsSelect{objects: objects, t: t, validate: func(request *QueryRequest) {
			...
			assert.NoError(t, err)
			assert.Equal(t, "1", result["outputList"].([]Object)[0]["v1"])
			assert.Equal(t, "2", result["outputList"].([]Object)[0]["v2"])
		})
		...
		t.Run("multi", func(t *testing.T) {
			cfg := &QueryConfig{
				...
				ClauseGroup: &ClauseGroup{
					Multi: &MultiClause{
						...
						Clauses: []*Clause{
							{Left: "v1", Operator: OperatorLike},
							{Left: "v2", Operator: OperatorLike},
						},
					},
				},
			...
			cfg.Queryer = &mockDsSelect{objects: objects, t: t, validate: func(request *QueryRequest) {
				if request.DatabaseInfoID != cfg.DatabaseInfoID {
					t.Fatal("database id should be equal")
		...
		t.Run("redundancy return field", func(t *testing.T) {
			...
			assert.NoError(t, err)
			fmt.Println(result)
			assert.Equal(t, int64(1), result["outputList"].([]Object)[0]["v1"])
			assert.Equal(t, int64(2), result["outputList"].([]Object)[0]["v2"])
			assert.Equal(t, nil, result["outputList"].([]Object)[0]["v3"])
	...
	t.Run("other case", func(t *testing.T) {
		...
		assert.Equal(t, int64(1), result["outputList"].([]Object)[0]["v1"])
		assert.Equal(t, 2.1, result["outputList"].([]Object)[0]["v2"])
		assert.Equal(t, false, result["outputList"].([]Object)[0]["v3"])
		assert.Equal(t, true, result["outputList"].([]Object)[0]["v4"])
		assert.Equal(t, "2020-02-20T10:10:10", result["outputList"].([]Object)[0]["v5"])
		assert.Equal(t, []int64{1, 2, 3}, result["outputList"].([]Object)[0]["v6"])
		assert.Equal(t, []bool{false, true, true}, result["outputList"].([]Object)[0]["v7"])
		assert.Equal(t, []float64{1.2, 2.1, 3.9}, result["outputList"].([]Object)[0]["v8"])
	...
	t.Run("config output list is nil", func(t *testing.T) {
		...
		result, err := ds.Query(context.Background(), cGroup)
		assert.NoError(t, err)
		assert.Equal(t, result["outputList"].([]Object)[0], Object{
			"v1": "1",
			"v2": "2.1",
			"v3": "0",
			"v4": "true",
