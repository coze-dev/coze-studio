...
export interface BatchImportAPIParams {
...
export const validateFiles = (selectedFiles: WorkflowFile[]): string[] => {
  ...
  validFiles.forEach(file => {
    ...
    if (nameSet.has(file.workflowName)) {
      nameErrors.push(
        t('workflow_name_duplicate', { workflowName: file.workflowName }),
      );
    }
...
export const callBatchImportAPI = async (
  params: BatchImportAPIParams,
): Promise<ImportResults> => {
  const { validFiles, spaceId, importMode, creatorId } = params;
  const workflowFiles = validFiles.map(file => {
    console.log('Processing file:', {
      ...
      isValidBase64: file.originalContent
        ? /^[A-Za-z0-9+/]*={0,2}$/.test(file.originalContent)
        : false,
  ...
  if (!response.ok) {
    ...
    try {
      ...
      throw new Error(
        t('batch_import_failed_http', { status: response.status }),
      );
    }
