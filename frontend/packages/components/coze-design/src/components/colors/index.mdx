...
# Coze Design Tokens
> Coze Design System 2.0 是由Bot Web的产品、设计师和前端团队，结合当前Coze产品的产品风格和视觉规范联合开发的一整套视觉设计系统，提供了包括设计规范（色彩、尺寸、字体）的基础预设。    —— [设计规范参考](https://bytedance.larkoffice.com/wiki/YT08wRAtbisNddkSOd8cF8K2nXe)
...
## 定制增强
在Coze的工程中，我们通过拓展了tailwind的config和plugin能力，将目前项目中预置的token直接通过编译构建的方式注入到了开发项目中，可以做到开箱即用，参考实践：[《Coze Design 2.0项目实践开发指南》](https://bytedance.larkoffice.com/wiki/FmnBwX2dGioWOEkKD6ucpcgynnh), 需要新增对应的token
只需要修改对应的：[config文件](https://code.byted.org/obric/bot-studio-monorepo/tree/master/config/tailwind-config/src)，增强内容如下：
...
- [《Parallel 组件库配置 使用说明》](https://bytedance.sg.larkoffice.com/docx/E3o7d4mn9os98Px3SM2lztAOgGc)
