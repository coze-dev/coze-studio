...
import { SliceCard } from '../slice-card/slice-card';
import { getDocumentOptions } from './utils';
...
export interface PreviewTextContentProps {
...
export const TextPreview = ({
  ...
  const { popoverNode, onContainerScroll, onContextMenu } = useContextMenu({
    canEdit: !!canEdit,
  });
  ...
  /** 判断是否有正在编辑的分段  */
  const isEditingSlice = useMemo(
    () => sliceData?.list?.some(item => !!item.id),
    [sliceData],
  );

  const insertEntry = (position, id?: string) => {
    if (isEditingSlice || isProcessing) {
      return;
    }
    const newList = sliceData?.list?.length ? [...sliceData.list] : [];
    if (position === 'last') {
      newList.push({
        id: nanoid(),
        sequence: '0',
      ...
      const index = newList.findIndex(item => item.slice_id === id);
      let sequence =
        newList.find(item => item.slice_id === id)?.sequence ?? '1';
      if (index !== -1) {
        const insterIndex = position === 'top' ? index : index + 1;
        sequence = position === 'top' ? sequence : String(Number(sequence) + 1);
        const slice = {
          id: nanoid(),
          sequence,
          content: '',
        };
        newList.splice(insterIndex, 0, slice);
      }
    }
    mutate({
      ...sliceData,
  ...
  const renderSliceCardList = () => {
    ...
    const renderData = sliceData?.list;
    ...
    return (
      <div className={styles['slice-article-content']}>
        {renderData?.map(slice => (
          <SliceCard
            isContentView
            onContextMenu={onContextMenu}
            // @ts-expect-error -- linter-disable-autofix
            canEdit={canEdit}
            isEditingSlice={isEditingSlice}
            key={slice?.slice_id}
            sliceInfo={slice}
            documentInfo={curDoc}
            dataSetDetail={dataSetDetail}
            curFormatType={curFormatType}
            initSliceList={(id: string) => {
              mutate({
                ...sliceData,
                total: Number(sliceData?.total ?? '0'),
                list: sliceData?.list?.filter(
                  lItem => lItem?.id !== id,
                ) as ISliceInfo[],
              });
            }}
            insertSuccess={sliceItem => {
              const list = sliceData?.list.length ? [...sliceData.list] : [];
              const index = list?.findIndex(v => !!v.id);
              if (index !== -1) {
                list.splice(index, 1, sliceItem);
                mutate({
                  ...sliceData,
                  total: Number(sliceData?.total ?? '0'),
                  list,
            ...
            onInsertEntry={position => insertEntry(position, slice.slice_id)}
            onDelete={sliceId => {
              mutate({
                ...sliceData,
                total: Number(sliceData?.total ?? '0'),
                list: sliceData?.list?.filter(
                  lItem => lItem.slice_id !== String(sliceId),
                ) as ISliceInfo[],
              });
              if (dataSetDetail) {
                setDataSetDetail({
            ...
            onUpdate={(sliceId, content) => {
              reload();
            }}
            onReload={() => reloadDataset?.()}
          />
        ))}
      </div>
    );
  ...
  const renderLevelSegments = () => {
    ...
    return (
      <TextSegmentTree
        content={getTreeNodes(levelSegments)}
        editable={canEdit}
        selectionIDs={selectionIDs}
        onDeleteSlice={sliceID => {
          setLevelSegments(
            levelSegments.filter(item => item.slice_id !== sliceID),
          );
        }}
      />
    );
    ...
    <>
      ...
      >
        ...
        <Spin
          ...
          size="large"
          wrapperClassName="h-full !w-full grow rounded-r-[8px]"
