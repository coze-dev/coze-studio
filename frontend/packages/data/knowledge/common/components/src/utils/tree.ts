...
export type TreeNode = Omit<ILevelSegment, 'children' | 'id' | 'parent'> & {
  id: string;
  parent: string;
  children: TreeNode[];
};
...
const getChildren = (
  ...
  (target.children ?? []).reduce<TreeNode[]>((acc, cur) => {
...
export const getTreeNodes = (segments: ILevelSegment[]): TreeNode[] => {
...
/**TreeNodes related */
export const findDescendantIDs = (node: TreeNode) => {
  const ids = new Set<string>();
  const findChild = (item: TreeNode) => {
    if (!item || !item.id) {
      return;
    }
    const { children } = item;
